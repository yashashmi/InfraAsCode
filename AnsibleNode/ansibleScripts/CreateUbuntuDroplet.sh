...
SIZE_NAME="1gb"
IMAGE_NAME="Ubuntu 20.04 (LTS) x64"

REGION_ID=`curl -X GET -H "Content-Type: application/json" -H "Authorization: Bearer $AUTH" "$BASE_URL/regions" | jq ".regions | map(select(.name==\"$REGION_NAME\"))[0].slug"`
...
#echo `curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $AUTH" -d '{"name":'$DROPLET_NAME',"region":'$REGION_ID',"size":'$SIZE_ID',"image":'$IMAGE_ID',"ssh_keys":'$SSH_KEY_ID',"backups":false,"ipv6":true,"user_data":null,"private_networking":null,"volumes": null,"tags":["web"]}' "$BASE_URL/droplets"`
